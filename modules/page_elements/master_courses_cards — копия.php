...
if($result2)
{
	...
	for($c = 0; $c < $rows; ++$c)
	{
		...
		$lessonsProgressQuery = "SELECT * FROM Lesson_progress JOIN Organized_course ON Lesson_progress.ID_organizedCourse=Organized_course.ID JOIN Course_lessons ON Organized_course.ID_course=Course_lessons.ID_course WHERE Organized_course.ID=$id_org_course AND Lesson_progress.isChecked=1";
		$lessonsProgressResult = mysqli_query($link, $lessonsProgressQuery) or die("Ошибка".mysqli_error($link));

		if($lessonsResult)
		{
			$rows3 = mysqli_num_rows($lessonsResult);
			for($l = 0; $l < $rows3; ++$l)
			{
				$lessonRow = mysqli_fetch_assoc($lessonsResult); 
				/*$lessonpProgressRow = mysqli_fetch_assoc($lessonsResult); */
				echo "<div class='course-lesson-item'>
						<input type='checkbox' class='course-lessons-checkboxes lessons-ckbx' name='lesson' value='".$lessonRow['ID']."'>
						<label for='lesson'>Занятие ".($l+1).". ".$lessonRow['title']."</label>
					</div>";
			}
		}
